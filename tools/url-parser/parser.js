...
import { renderHistory, saveToHistory, clearHistory } from "./js/history.js";

// --- Global UI Listeners ---

document
  .getElementById("parseBtn")
  .addEventListener("click", () => parseUrl(false));
...
document.getElementById("clearInputBtn").addEventListener("click", () => {
  ...
  ["protocol", "host", "port", "hash"].forEach(
  ...
  document.getElementById("pathnameContainer").innerHTML =
    '<span class="sub-text" style="font-style: italic; opacity: 0.6">Click to edit segments...</span>';
  handleYouTubeUrl();
  input.focus();
});
...
document.getElementById("copyBtn").addEventListener("click", () => {
  ...
  navigator.clipboard.writeText(copyText.value).then(() => {
    ...
    btn.textContent =
      currentLocale === "zh-HK" || currentLocale === "zh-Hant"
        ? "已複製!"
        : "Copied!";
...
document
  .getElementById("cleanYoutubeBtn")
  .addEventListener("click", cleanYouTubeUrl);
...
["ytHours", "ytMinutes", "ytSeconds"].forEach((id) => {
  ...
  input.addEventListener("keypress", (e) => {
    if (e.key === "Enter") applyYouTubeTimestamp();
  });
